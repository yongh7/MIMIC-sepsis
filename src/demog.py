...
# Charlson Comorbidity Index, query from https://github.com/MIT-LCP/mimic-code/blob/main/mimic-iv/concepts/comorbidity/charlson.sql
query = """
DROP MATERIALIZED VIEW IF EXISTS PUBLIC.CHARLSON CASCADE;
CREATE MATERIALIZED VIEW PUBLIC.CHARLSON AS
WITH diag AS (
    SELECT
        hadm_id
...
, ag AS (
    SELECT
        ...
        , CASE 
            WHEN (pa.anchor_age + EXTRACT(YEAR FROM ad.admittime) - pa.anchor_year) <= 50 THEN 0
            WHEN (pa.anchor_age + EXTRACT(YEAR FROM ad.admittime) - pa.anchor_year) <= 60 THEN 1
            WHEN (pa.anchor_age + EXTRACT(YEAR FROM ad.admittime) - pa.anchor_year) <= 70 THEN 2
            WHEN (pa.anchor_age + EXTRACT(YEAR FROM ad.admittime) - pa.anchor_year) <= 80 THEN 3
            ELSE 4 
        END AS age_score
    FROM mimiciv_hosp.admissions ad
...
# Update query to use the new comorbidity view
query = """
SELECT ad.subject_id, ad.hadm_id, i.stay_id
    ...
    , CAST(extract(epoch from age(p.dod,ad.dischtime))<=24*3600 as int) as morta_hosp
...
INNER JOIN mimiciv_icu.icustays i
    ON ad.hadm_id = i.hadm_id
INNER JOIN mimiciv_hosp.patients p
    ON p.subject_id = i.subject_id
LEFT JOIN PUBLIC.CHARLSON com
    ON ad.hadm_id = com.hadm_id
